...
class MriAnnotationService:
    ...
    def create_rectangle_annotation(
        ...
        """Creates a clinician rectangle annotation on an MRI scan."""
        # 1. Check Authentication and Role
        if not actor:
            raise MriAnnotationServiceException("Authentication required.")
        ...
        try:
            if x is None or y is None or width is None or height is None:
                raise ValueError()
            if isinstance(x, bool) or isinstance(y, bool) or isinstance(width, bool) or isinstance(height, bool):
                raise ValueError()
            x_val = float(x)
            y_val = float(y)
            w_val = float(width)
        ...
        if math.isnan(x_val) or math.isinf(x_val) or math.isnan(y_val) or math.isinf(y_val) or \
           math.isnan(w_val) or math.isinf(w_val) or math.isnan(h_val) or math.isinf(h_val):
            raise MriAnnotationServiceException("Coordinates cannot be NaN or Infinity.")

        if x_val < 0.0 or x_val > 1.0 or y_val < 0.0 or y_val > 1.0:
            raise MriAnnotationServiceException("Coordinates must be normalized between 0.0 and 1.0.")

        if w_val <= 0.0 or h_val <= 0.0 or w_val > 1.0 or h_val > 1.0:
            raise MriAnnotationServiceException("Width and height must be greater than 0.0 and less than or equal to 1.0.")

        if x_val + w_val > 1.0:
            raise MriAnnotationServiceException("Rectangle x + width exceeds 1.0.")

        if y_val + h_val > 1.0:
            raise MriAnnotationServiceException("Rectangle y + height exceeds 1.0.")
        ...
        # 7. Enforce Security Access Checks
        if not self.auth_svc.can_access_patient(actor, patient_id):
            self._log_audit_event("RECTANGLE_ANNOTATION_CREATED", actor, "FAILED", f"Access denied creating annotation for patient: {patient_id}")
            raise MriAnnotationServiceException("Access denied.")

        if not self.auth_svc.can_access_scan(actor, scan_id):
            self._log_audit_event("RECTANGLE_ANNOTATION_CREATED", actor, "FAILED", f"Access denied creating annotation for scan: {scan_id}")
            raise MriAnnotationServiceException("Access denied.")
        ...
        try:
            with conn:
                cursor = conn.cursor()
                cursor.execute(
                    """INSERT INTO mri_rectangle_annotations (scan_id, patient_id, doctor_id, x_normalized, y_normalized, width_normalized, height_normalized, label, encrypted_comment, created_at, updated_at, status)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active');""",
                    (scan_id, patient_id, actor.id, x_val, y_val, w_val, h_val, trimmed_label, encrypted_comment, now, now)
                )
                annotation_id = cursor.lastrowid
        finally:
            conn.close()
        ...
        self._log_audit_event(
            "RECTANGLE_ANNOTATION_CREATED",
            actor,
            "SUCCESS",
            f"Created rectangle annotation ID {annotation_id} for scan {scan_id} at ({x_val}, {y_val}, {w_val}, {h_val})."
        )
    ...
    def get_rectangle_annotations_for_scan(
        self,
        actor: User,
        scan_id: int
    ) -> List[Dict[str, Any]]:
        """Retrieves active rectangle annotations for a given MRI scan."""
        if not actor:
            raise MriAnnotationServiceException("Authentication required.")
        ...
        try:
            rows = conn.execute(
                """SELECT a.*, u.full_name as doctor_name, u.email as doctor_email
                   FROM mri_rectangle_annotations a
                   JOIN users u ON a.doctor_id = u.id
                   WHERE a.scan_id = ? AND a.status = 'active'
                   ORDER BY a.created_at ASC;""",
                (scan_id,)
        ...
        for r in rows:
            ...
            results.append({
                ...
                "x": r["x_normalized"],
                "y": r["y_normalized"],
                "width": r["width_normalized"],
                "height": r["height_normalized"],
                "label": r["label"],
                "comment": decrypted_comment,
                "created_at": r["created_at"],
                "updated_at": r["updated_at"],
    ...
    def update_rectangle_annotation(
        self,
        actor: User,
        annotation_id: int,
        x: float,
        ...
        """Updates an existing clinician rectangle annotation."""
        if not actor:
            raise MriAnnotationServiceException("Authentication required.")
        ...
        conn = self._get_connection()
        try:
            row = conn.execute("SELECT * FROM mri_rectangle_annotations WHERE annotation_id = ?;", (annotation_id,)).fetchone()
            if not row:
                raise MriAnnotationServiceException("Annotation not found.")
        ...
        # Ownership enforcement (only creator can update)
        if doctor_id != actor.id:
            self._log_audit_event("RECTANGLE_ANNOTATION_UPDATED", actor, "FAILED", f"Unauthorized attempt by user {actor.id} to update annotation ID {annotation_id} authored by {doctor_id}.")
            raise MriAnnotationServiceException("Access denied.")
        ...
        try:
            with conn:
                conn.execute(
                    """UPDATE mri_rectangle_annotations
                       SET x_normalized = ?, y_normalized = ?, width_normalized = ?, height_normalized = ?, label = ?, encrypted_comment = ?, updated_at = ?
                       WHERE annotation_id = ?;""",
                    (x_val, y_val, w_val, h_val, trimmed_label, encrypted_comment, now, annotation_id)
                )
        finally:
            conn.close()

        self._log_audit_event(
            "RECTANGLE_ANNOTATION_UPDATED",
            actor,
            "SUCCESS",
            f"Updated rectangle annotation ID {annotation_id} for scan {scan_id}."
        )
    ...
    def archive_rectangle_annotation(
        self,
        actor: User,
        annotation_id: int
    ) -> Dict[str, Any]:
        """Soft-archives/deletes an existing rectangle annotation."""
        if not actor:
            raise MriAnnotationServiceException("Authentication required.")
        ...
        # Ownership check
        if doctor_id != actor.id:
            self._log_audit_event("RECTANGLE_ANNOTATION_ARCHIVED", actor, "FAILED", f"Unauthorized attempt by user {actor.id} to archive annotation ID {annotation_id} authored by {doctor_id}.")
            raise MriAnnotationServiceException("Access denied.")
        ...
        self._log_audit_event(
            "RECTANGLE_ANNOTATION_ARCHIVED",
            actor,
            "SUCCESS",
            f"Archived rectangle annotation ID {annotation_id}."
        )
